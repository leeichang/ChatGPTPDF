...
import { useAppStore } from "@/store";
    ...
    const dragButtonRef = ref(null);
      ...
      const dragButton = dragButtonRef.value;

      if (!container || !dragButton) return;
      ...
      const mouseMoveHandler = (event: MouseEvent) => {
        ...
        const leftDivWidth = containerWidth / 2 + diffX;
        const rightDivWidth = containerWidth / 2 - diffX;

        container.style.gridTemplateColumns = `${leftDivWidth}px ${rightDivWidth}px`;
				dragButton.style.left = `${leftDivWidth}px`;
      };
      ...
      document.addEventListener('mouseup', mouseUpHandler);
		...
		onMounted(() => {
			...
			const dragButton = dragButtonRef.value;
      dragButton?.addEventListener('mousedown', startResize);
		});

		onUnmounted(() => {
			if (loading.value) controller.abort();
			const dragButton = dragButtonRef.value;
      dragButton?.removeEventListener('mousedown', startResize);
		});
...
<template>
	<div class="container" ref="containerRef">
		...
		<div class="drag-button" ref="dragButtonRef" @mousedown="startResize"></div>
		<div class="right-div">
