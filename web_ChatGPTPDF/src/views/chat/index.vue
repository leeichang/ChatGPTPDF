...
const leftWidth = ref<number>(600);
const rightWidth = ref<number>(600);
...
const updateLayout = (e: MouseEvent) => {
  if (isDragging) {
    const containerRect = container.value!.getBoundingClientRect();
    const dividerRect = divider.value!.getBoundingClientRect();
    const newLeftWidth = e.clientX - containerRect.left;
    const newRightWidth = containerRect.right - e.clientX;
    leftWidth.value = newLeftWidth;
    rightWidth.value = newRightWidth;
    divider.value!.style.left = newLeftWidth + "px";
  }
};

window.addEventListener("mouseup", stopDragging);
window.addEventListener("mousemove", updateLayout);
