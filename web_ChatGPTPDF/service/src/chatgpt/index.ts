...
if (process.env.TIMEOUT_MS) {
  timeoutMs = !isNaN(+process.env.TIMEOUT_MS)
...
(async () => {
  ...
  if (isNotEmptyString(process.env.OPENAI_API_KEY)) {
    ...
    const model = isNotEmptyString(OPENAI_API_MODEL)
      ...
      : "gpt-3.5-turbo";
    var open_ai_key = "";
    if (process.env.OPENAI_API_KEY) open_ai_key = process.env.OPENAI_API_KEY;
    else open_ai_key = "";
    ...
    if (process.env.OPENAI_ACCESS_TOKEN)
      access_token = process.env.OPENAI_ACCESS_TOKEN;
    else access_token = "";
