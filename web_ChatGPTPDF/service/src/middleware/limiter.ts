...
var MAX_REQUEST_PER_HOUR = "";
if (process.env.MAX_REQUEST_PER_HOUR)
  MAX_REQUEST_PER_HOUR = process.env.MAX_REQUEST_PER_HOUR;
...
const limiter = rateLimit({
  ...
  message: async (req: any, res: { send: (arg0: { status: string; message: string; data: null; }) => void; }) => {
    res.send({
      status: "Fail",
